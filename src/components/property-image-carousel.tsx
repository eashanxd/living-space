...
export function PropertyImageCarousel({ images, title, variant, overlay }: PropertyImageCarouselProps) {
  const [carouselState, setCarouselState] = useState({ imageCount: images.length, activeImageIndex: 0 });
  const pointerStart = useRef<number | null>(null);
  const activeImageIndex = carouselState.imageCount === images.length ? carouselState.activeImageIndex : 0;
  const hasMultipleImages = images.length > 1;

  const showPrevious = () => setCarouselState({ imageCount: images.length, activeImageIndex: (activeImageIndex - 1 + images.length) % images.length });
  const showNext = () => setCarouselState({ imageCount: images.length, activeImageIndex: (activeImageIndex + 1) % images.length });
    ...
    >
      {images.length ? (
        <div
          ...
          role="img"
          aria-label={`${title} image ${activeImageIndex + 1} of ${images.length}`}
        />
      ) : (
        ...
        <div className="absolute inset-x-4 bottom-4 z-10 flex gap-2 overflow-x-auto pb-1" onPointerDown={(event) => event.stopPropagation()}>
          {images.map((image, index) => (
            <button
              ...
              onPointerDown={(event) => event.stopPropagation()}
              onClick={() => setCarouselState({ imageCount: images.length, activeImageIndex: index })}
              className={`h-12 w-16 shrink-0 overflow-hidden rounded-lg border-2 bg-[#e9e1d8] transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white ${index === activeImageIndex ? "border-white" : "border-white/40 opacity-75 hover:opacity-100"}`}
